...
def configure_vpc_routes(region, vpc_resources, tgw_id, vpc_configs):
    ...
    for i, resource in enumerate(vpc_resources):
        ...
        current_vpc_name = vpc_configs[i]['name']
        
        print(f"Configurando rutas para {current_vpc_name}...")
        
        # 1. RUTA CROSS-REGION (Original)
        if region == 'us-east-1':
            cross_region_cidr = '192.168.0.0/16'
        else:
            cross_region_cidr = '10.0.0.0/8'
            
        try:
            ec2.create_route(RouteTableId=rt_id, DestinationCidrBlock=cross_region_cidr, TransitGatewayId=tgw_id)
            print(f"  Ruta Cross-Region {cross_region_cidr} -> TGW configurada")
        except Exception as e:
            if "RouteAlreadyExists" not in str(e): print(f"  Error ruta cross: {e}")
        
        # 2. RUTA INTRA-REGION (La corrección necesaria)
        # Recorremos todas las VPCs de esta región para añadir rutas hacia las vecinas
        for config in vpc_configs:
            neighbor_name = config['name']
            neighbor_cidr = config['vpc_cidr']
            
            # Si no soy yo mismo, añado una ruta hacia mi vecino apuntando al TGW
            if neighbor_name != current_vpc_name:
                try:
                    ec2.create_route(
                        RouteTableId=rt_id, 
                        DestinationCidrBlock=neighbor_cidr, 
                        TransitGatewayId=tgw_id
                    )
                    print(f"  Ruta Local a {neighbor_name} ({neighbor_cidr}) -> TGW configurada")
                except Exception as e:
                    if "RouteAlreadyExists" not in str(e): print(f"  Error ruta local: {e}")
        time.sleep(5)
...
def main():
    ...
    try:
        ...
        print("🔗 Conectividad entre todas las VPCs habilitada")
